...
public class CadesService
{
        ...
        SignatureParameters parameters, CmsSignedData originalSignedData)
    {
        ...
        CmsAttributeTableGenerator signedAttrGen = new DefaultSignedAttributeTableGenerator
                (new BcCms.AttributeTable(signedAttrDic));
    ...
    private BcCms.Attribute MakeSigningCertificateAttribute(SignatureParameters parameters)
    {
        ...
        byte[] certHash = DigestUtilities.CalculateDigest(parameters.DigestMethod.Name.ToString(), certificate.GetEncoded());
        ...
        if (parameters.DigestMethod == DigestMethod.SHA1)
        {
            ...
            if (policies != null)
            {
                ...
                {
                    new DerSequence(new EssCertID(certHash, issuerSerial)),
                    new DerSequence(policies)
                };
                sc = SigningCertificate.GetInstance(new DerSequence(vector));
            }
            else
            {
        ...
        {
            EssCertIDv2 essCert = new EssCertIDv2(new AlgorithmIdentifier(parameters.DigestMethod.Oid), certHash, issuerSerial);

            SigningCertificateV2 scv2 = new SigningCertificateV2(new EssCertIDv2[] { essCert }, policies);

            return new BcCms.Attribute(PkcsObjectIdentifiers.IdAASigningCertificateV2, new DerSet(scv2));
        }
    }
    ...
    private BcCms.Attribute MakeSignaturePolicyAttribute(SignatureParameters parameters)
    {
        SignaturePolicyIdentifier sigPolicy = new SignaturePolicyIdentifier(
            new SignaturePolicyId(
                new DerObjectIdentifier(parameters.SignaturePolicyInfo.PolicyIdentifier), 
                new OtherHashAlgAndValue(
                    new AlgorithmIdentifier(parameters.SignaturePolicyInfo.PolicyDigestAlgorithm.Oid), 
                    new DerOctetString(Convert.FromBase64String(parameters.SignaturePolicyInfo.PolicyHash)))));
    ...
    private Dictionary<DerObjectIdentifier, Asn1Encodable> GetSignedAttributes(SignatureParameters parameters)
    {
        ...
        signedAttrs.Add(signingCertificateReference.AttrType,
