...
//						Asn1.Cms.AttributeTable unsigned = _unsAttr.getAttributes(Collections.unmodifiableMap(parameters));
		...
		*/
			...
			DerObjectIdentifier contentOid)
		{
            ...
            if (attrCertV1Found || !CmsObjectIdentifiers.Data.Equals(contentOid) || CheckForVersion3(signers))
            {
            ...
            : BaseOutputStream
        {
            ...
            private void DoClose()
            {
				...
				outer.digests.Clear();    // clear the current preserved digest state

				if (outer.certs.Count > 0)
				{
					Asn1Set certs = CmsUtilities.CreateBerSetFromList(outer.certs);

					WriteToGenerator(_sigGen, new BerTaggedObject(false, 0, certs));
				}

				if (outer.crls.Count > 0)
				{
					Asn1Set crls = CmsUtilities.CreateBerSetFromList(outer.crls);
                ...
                //
				{
					foreach (DigestAndSignerInfoGeneratorHolder holder in outer._signerInfs)
					{
						...
						outer.digests[holder.digestOID] = calculatedDigest.Clone();
                ...
                //
				{
					foreach (SignerInformation signer in outer.signers)
					{
